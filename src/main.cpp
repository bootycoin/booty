...
#define BLOCK_COINS 5000
#define GENESIS_TIME 1390700293
#define BLOCK_NONCE  3847456
#define BLOCK_NONCE_TESTNET 3847456
...
uint256 hashGenesisBlock("0xcdb32016c8194e52f087ef926b7473fc6b6a6d8937a67249d3c3725a394c5a9e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Booty: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Booty Signed Message:\n";
...
{
    // Booty: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = BLOCK_COINS * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Booty: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6 * 60 * 60; // Booty: 6 hours
static const int64 nTargetSpacing = 60; // Booty: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Booty: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Booty: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Booty: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=GENESIS_TIME, nBits=1e0ffff0, nNonce=2084524493, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Nothing Ventured, Nothing Gained";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = BLOCK_COINS * COIN;
        ...
        block.nNonce   = BLOCK_NONCE;

        if (fTestNet)
        {
            block.nTime    = GENESIS_TIME;
            block.nNonce   = BLOCK_NONCE_TESTNET;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe3c774349421a018cba788180e8e3a2c2279327cb1156996598d7973266a763c"));
#if 0
/********************** BEGIN GENESIS BLOCK GENERATOR ***************************/
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Booty: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BootyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BootyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BootyMiner(CWallet *pwallet)
{
    printf("BootyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("booty-miner");
    ...
    try { loop {
        ...
        printf("Running BootyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BootyMiner, pwallet));
}
